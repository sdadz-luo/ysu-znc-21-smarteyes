...
float pid_location_weizi(pid* pid_struct,float now_value){
    ...
    static float kkp = 1;
    if (fabs(pid_struct->error) <= 30) kkp = 0.6;
    else kkp = 1;
...
void pid_init(void){
	...
	PID_init(&pid_x,4,0.001,10,500,70,0,0.5);
	PID_init(&pid_y,-4,-0.001,-10,500,70,0,0.5);
	PID_init(&pid_yaw,3,0.001,7,1000,60,0,0.1);
